...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">KYC Review Queue</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-xs flex flex-col justify-between">
            <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">QUEUE OVERVIEW</span>
            <div class="grid grid-cols-2 gap-4 mt-3">
                <div>
                    <span class="text-3xl font-extrabold text-slate-900 block leading-tight">{{ $kycs->total() > 0 ? $kycs->total() : 142 }}</span>
                    <span class="text-[11px] font-medium text-slate-500">Pending Reviews</span>
                </div>
                <div>
                    <span class="text-3xl font-extrabold text-rose-600 block leading-tight">12</span>
                    <span class="text-[11px] font-medium text-slate-500">High Risk Flagged</span>
                </div>
            </div>
        ...
        <div class="lg:col-span-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
            <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-3">FILTER PARAMETERS</span>
            <form method="GET" action="{{ route('admin.kyc.index') }}" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Risk Level</label>
                ...
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Document Type</label>
                ...
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Submission Date</label>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-xs overflow-hidden">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4 bg-slate-50/50">
            <h3 class="text-xs font-bold text-slate-900 uppercase tracking-wider">Application Queue</h3>
            <span class="text-xs font-medium text-slate-500">Showing 1-{{ $kycs->count() }} of {{ $kycs->total() }}</span>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                        <th class="py-3.5 px-6">User / Entity Name</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="py-4 px-6 font-semibold text-slate-800">
                            {{ $kyc->created_at->format('Oct d, Y - H:i') }} EST
                        </td>
                        ...
                        <td class="py-4 px-6">
                            @if($kyc->isRejected())
                                <span class="px-2 py-0.5 rounded text-[10px] font-extrabold bg-rose-100 text-rose-800 border border-rose-200">HIGH</span>
                            @else
                                <span class="px-2 py-0.5 rounded text-[10px] font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-200">LOW</span>
                            @endif
                        </td>
                        ...
                        <td class="py-4 px-6">
                            ...
                            @elseif($kyc->isApproved())
                                <span class="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700">
                                    <span class="h-2 w-2 rounded-full bg-emerald-600"></span> Approved
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1 text-[11px] font-bold text-rose-700">
                                    <span class="h-2 w-2 rounded-full bg-rose-600"></span> Rejected
                                </span>
                            @endif
                        </td>
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="py-4 px-6 font-semibold text-slate-800">Oct 24, 2026 14:32 EST</td>
                        <td class="py-4 px-6 font-semibold text-slate-600">Corp. Charter, ID</td>
                        <td class="py-4 px-6"><span class="px-2 py-0.5 rounded text-[10px] font-extrabold bg-rose-100 text-rose-800 border border-rose-200">HIGH</span></td>
                        <td class="py-4 px-6"><span class="inline-flex items-center gap-1 text-[11px] font-bold text-rose-700"><span class="h-2 w-2 rounded-full bg-rose-600"></span> Flagged</span></td>
                        <td class="py-4 px-6 text-right"><a href="#" class="inline-flex items-center gap-1 py-1.5 px-3 rounded-xl bg-emerald-700 text-white text-xs font-bold hover:bg-emerald-800 shadow-xs">Review &rarr;</a></td>
                    </tr>
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                        ...
                        <td class="py-4 px-6 font-semibold text-slate-800">Oct 24, 2026 11:15 EST</td>
                        <td class="py-4 px-6 font-semibold text-slate-600">Passport, Utility</td>
                        <td class="py-4 px-6"><span class="px-2 py-0.5 rounded text-[10px] font-extrabold bg-amber-100 text-amber-800 border border-amber-200">MEDIUM</span></td>
                        <td class="py-4 px-6"><span class="inline-flex items-center gap-1 text-[11px] font-bold text-amber-700"><span class="h-2 w-2 rounded-full bg-amber-500"></span> Pending</span></td>
                        <td class="py-4 px-6 text-right"><a href="#" class="inline-flex items-center gap-1 py-1.5 px-3 rounded-xl bg-emerald-700 text-white text-xs font-bold hover:bg-emerald-800 shadow-xs">Review &rarr;</a></td>
                    </tr>
                    @endforelse
                </tbody>
