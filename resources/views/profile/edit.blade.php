...
<div class="space-y-6 max-w-6xl mx-auto" x-data="{ profileTab: '{{ session('tab') ?? request('tab') ?? 'personal' }}', colorTheme: localStorage.getItem('lendflow_theme') || '{{ $user->profile?->system_preferences['color_theme'] ?? 'light' }}', density: localStorage.getItem('lendflow_density') || '{{ $user->profile?->system_preferences['data_density'] ?? 'comfortable' }}', disable2faModalOpen: false, generateTokenModalOpen: false }">
    ...
    <div x-show="profileTab === 'system'" class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xs space-y-6" style="display: none;">
        ...
        <form action="{{ route('profile.system.update') }}" method="POST" class="space-y-6">
            ...
            <!-- Appearance Settings -->
            <div class="p-6 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label @click="colorTheme = 'light'; applyTheme('light')"
                                   :class="colorTheme === 'light' ? 'border-emerald-700 dark:border-emerald-500 bg-white dark:bg-slate-900/80 shadow-xs ring-2 ring-emerald-600/20' : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/40 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'"
                                   class="p-3.5 rounded-xl border flex items-center justify-between cursor-pointer transition-all select-none">
                                <div class="flex items-center gap-2.5">
                                    <div class="p-2 rounded-lg bg-amber-50 dark:bg-amber-950/40 text-amber-500 border border-amber-200/60 dark:border-amber-900/40">
                                ...
                                <input type="radio" id="radio_theme_light" name="color_theme" value="light" @change="colorTheme = 'light'; applyTheme('light')" {{ ($sysSettings['color_theme'] ?? 'light') === 'light' ? 'checked' : '' }} class="accent-emerald-700">
                            </label>

                            <label @click="colorTheme = 'dark'; applyTheme('dark')"
                                   :class="colorTheme === 'dark' ? 'border-emerald-700 dark:border-emerald-500 bg-white dark:bg-slate-900/80 shadow-xs ring-2 ring-emerald-600/20' : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/40 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'"
                                   class="p-3.5 rounded-xl border flex items-center justify-between cursor-pointer transition-all select-none">
                                <div class="flex items-center gap-2.5">
                                    <div class="p-2 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 text-indigo-500 border border-indigo-200/60 dark:border-indigo-900/40">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                ...
                                <input type="radio" id="radio_theme_dark" name="color_theme" value="dark" @change="colorTheme = 'dark'; applyTheme('dark')" {{ ($sysSettings['color_theme'] ?? 'light') === 'dark' ? 'checked' : '' }} class="accent-emerald-700">
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label @click="density = 'comfortable'; applyDensity('comfortable')"
                                   :class="density === 'comfortable' ? 'border-emerald-700 dark:border-emerald-500 bg-white dark:bg-slate-900/80 shadow-xs ring-2 ring-emerald-600/20' : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/40 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'"
                                   class="p-3.5 rounded-xl border flex items-center justify-between cursor-pointer transition-all select-none">
                                <div>
                            ...
                            <label @click="density = 'compact'; applyDensity('compact')"
                                   :class="density === 'compact' ? 'border-emerald-700 dark:border-emerald-500 bg-white dark:bg-slate-900/80 shadow-xs ring-2 ring-emerald-600/20' : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/40 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'"
                                   class="p-3.5 rounded-xl border flex items-center justify-between cursor-pointer transition-all select-none">
                                <div>
                    ...
                    <div>
                        ...
                        <select id="language_select" onchange="window.location.href='/lang/' + this.value + '?redirect=' + encodeURIComponent(window.location.href)" class="w-full sm:w-72 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3.5 py-2.5 text-xs font-semibold text-slate-900 dark:text-slate-100 outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-xs cursor-pointer">
                        ...
                        <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium mt-1">{{ __('Select your preferred system interface language across all pages.') }}</p>
                    </div>
                </div>
            </div>
