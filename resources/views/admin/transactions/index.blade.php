...
@section('title', __('Transaction Monitoring') . ' - Admin Terminal')
...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">{{ __('Transaction Monitoring') }}</h1>
            <p class="text-xs font-medium text-slate-500 mt-1">{{ __('Real-time audit log of all platform deposits, withdrawals, disbursements, and repayments.') }}</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                        ...
                        <th class="py-3.5 px-6">{{ __('AMOUNT (USD)') }}</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="py-4 px-6 font-bold text-slate-900">{{ $tx->wallet?->user?->email ?? __('System Node') }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="py-12 text-center text-slate-400 font-medium">{{ __('No transaction audit logs recorded.') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
