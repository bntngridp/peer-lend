...
<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="px-6 py-6 sm:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <div class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-900 dark:text-slate-100 mt-1">
                        <span class="h-2 w-2 rounded-full shrink-0
                            @if($loan->status === 'pending' || $loan->status === 'open_funding') bg-amber-500
                            @elseif($loan->status === 'active' || $loan->status === 'funded') bg-emerald-500
                            @elseif($loan->status === 'completed') bg-slate-400
                            @else bg-rose-500 @endif"></span>
                        <span class="capitalize">{{ __(str_replace('_', ' ', $loan->status)) }}</span>
                    </div>
                </div>
            </div>
    ...
    @elseif(in_array($loan->status, ['active', 'completed', 'defaulted']))
        <div class="shadow-xs dark:shadow-none rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider">{{ __('Repayment & Collection Monitoring') }}</h3>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ __('Track installment amortization schedules, penalty fees, payment receipts, and digital contracts.') }}</p>
            </div>
            <div class="flex items-center gap-3 flex-wrap">
                <a href="{{ route('loans.installments', $loan->id) }}" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 text-xs font-bold transition-all shadow-xs">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>
                    <span>{{ __('View Installment Schedule') }}</span>
                </a>
                <a href="{{ route('loans.agreement', $loan->id) }}" target="_blank" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 px-4 py-2.5 text-xs font-bold hover:bg-slate-100 dark:hover:bg-slate-700 transition-all shadow-xs">
