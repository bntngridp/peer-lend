...
<div class="min-h-[80vh] flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <div class="bg-white px-8 py-10 shadow-xs rounded-2xl border border-slate-200 text-center space-y-6">
            ...
            <!-- Title & Subtitle -->
            <div>
                <h2 class="text-xl font-extrabold text-slate-900 tracking-tight">{{ __('Two-Factor Verification') }}</h2>
                <p class="text-xs text-slate-500 mt-1.5 leading-relaxed font-medium">
                    {{ __('Enter the 6-digit code from your :app / :authy app to continue signing into LendFlow.', ['app' => 'Google Authenticator', 'authy' => 'Authy']) }}
                </p>
            </div>
            ...
            <form action="{{ route('2fa.verify.post') }}" method="POST" class="space-y-5">
                ...
                <!-- OTP Input -->
                <div>
                    <label for="code" class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">{{ __('6-Digit OTP Code') }}</label>
            ...
            <div class="mt-6 pt-4 border-t border-slate-100 text-[11px] text-slate-400 font-medium">
                {{ __('Secured by LendFlow Institutional Grade 2FA Encryption') }}
            </div>
